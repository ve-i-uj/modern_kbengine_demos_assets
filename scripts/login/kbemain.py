...
def onLoginAppReady():
    ...
    """
    log.setup()
    logger.info('onLoginAppReady: bootstrapGroupIndex=%s, bootstrapGlobalIndex=%s' %
...
def onLoginAppShutDown():
    ...
    这个loginapp被关闭前的回调函数
    """
    logger.info('onLoginAppShutDown()')
...
def onRequestLogin(loginName, password, clientType, datas):
    ...
    """
    logger.info('onRequestLogin() loginName=%s, clientType=%s' % (loginName, clientType))
...
def onLoginCallbackFromDB(loginName, accountName, errorno, datas):
    ...
    """
    logger.info('onLoginCallbackFromDB() loginName=%s, accountName=%s, errorno=%s' % (loginName, accountName, errorno))
...
def onRequestCreateAccount(accountName, password, datas):
    ...
    请求账号创建时回调
    """
    logger.info('onRequestCreateAccount() %s' % (accountName))
...
def onCreateAccountCallbackFromDB(accountName, errorno, datas):
    ...
    errorno: KBEngine.SERVER_ERR_*
    """
    logger.info('onCreateAccountCallbackFromDB() accountName=%s, errorno=%s' % (accountName, errorno))
