...
def onInterfaceAppReady():
    ...
    """
    log.setup()
    logger.info('onInterfaceAppReady: bootstrapGroupIndex=%s, bootstrapGlobalIndex=%s' %
...
def onInterfaceAppShutDown():
    ...
    """
    logger.info('onInterfaceAppShutDown()')
    g_poller.stop()
...
def onRequestCreateAccount(registerName, password, datas):
    ...
    @type  datas: bytes
    """
    logger.info('onRequestCreateAccount: registerName=%s' % (registerName))
...
def onRequestAccountLogin(loginName, password, datas):
    ...
    @type  datas: bytes
    """
    logger.info('onRequestAccountLogin: registerName=%s' % (loginName))
...
def onRequestCharge(ordersID, entityDBID, datas):
    ...
    @type  datas: bytes
    """
    logger.info('onRequestCharge: entityDBID=%s, entityDBID=%s' % (ordersID, entityDBID))
...
def onSqlCallback(result, rows, insertid, error):
    logger.debug('onSqlCallback: result=%s, rows=%s, insertid=%s, error=%s' %
                 (str(result), str(rows), str(insertid), str(error)))


def onHttpCallback(httpcode, data, headers, success, url):
    logger.debug('onHttpCallback: httpcode=%i, data=%s, headers=%s, success=%s, url=%s' %
